...
@click.option('--dist', '-d', default='assets', type=click.Path(exists=False), help='出力先のディレクトリパス')
def build_index(owl_data_ttl, dist):
    if not owl_data_ttl:
        raise click.UsageError('オントロジーファイルを一つ以上指定してください。')

    for owl_data in owl_data_ttl:
        if get_type(owl_data) not in ('turtle', 'n3'):
            raise click.UsageError('オントロジーファイルはttlかn3のみ有効です。')
...
@click.option('--assets', '-a', type=click.Path(exists=True), help='build_indexコマンドで吐き出されたディレクトリ')
@click.option('--dist', '-d', default='model.json', type=click.Path(exists=False), help='出力先のファイルパス')
